...
def get_current_branch(cwd: Optional[str] = None, timeout: int = 30) -> Optional[str]:
    """Get current git branch name.

    Args:
        cwd: Working directory for git command
        timeout: Maximum seconds to wait for git command (default 30)

    Returns:
        Branch name or None if command fails/times out
    """
    try:
        ...
        # Command hung - likely filesystem or antivirus issue
        return None
    except subprocess.CalledProcessError:
        # Git command failed (not in a git repo, detached HEAD, etc.)
        return None
    except Exception:
...
def push_branch(
    ...
    try:
        result = subprocess.run(
            ...
            text=True,
            cwd=cwd,
            timeout=180,  # 3 minutes for push
            check=True
        )
        return True, None
    except subprocess.TimeoutExpired:
        return False, "Git push timed out after 180s - check network connection or repository size"
...
def commit_changes(
    ...
    try:
        ...
        return False, f"Git operation timed out after {e.timeout}s - this may indicate filesystem or antivirus issues"
    except subprocess.CalledProcessError as e:
        return False, e.stderr if e.stderr else str(e)
    except Exception as e:
        return False, str(e)
...
def finalize_git_operations(
    ...
    if not branch_name:
        ...
        current_branch = get_current_branch(cwd=cwd, timeout=30)
        if current_branch and current_branch != "main":
            ...
            logger.error(
                "No branch name in state and could not determine current branch, skipping git operations"
            )
            return
        else:
            logger.error(
