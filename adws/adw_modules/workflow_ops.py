...
from adw_modules.utils import parse_json, get_local_timestamp
...
def format_issue_message(
    ...
    """Format a message for issue comments with ADW tracking, timestamp, and bot identifier."""
    timestamp = get_local_timestamp()
    # Always include ADW_BOT_IDENTIFIER to prevent webhook loops
    if session_id:
        return f"{ADW_BOT_IDENTIFIER} {adw_id}_{agent_name}_{session_id} [{timestamp}]: {message}"
    return f"{ADW_BOT_IDENTIFIER} {adw_id}_{agent_name} [{timestamp}]: {message}"


def post_workflow_completion_summary(
    issue_number: str,
    adw_id: str,
    workflow_name: str,
    status: str = "success",
    ...
    """Post a formatted completion summary to a GitHub issue.

    Args:
        issue_number: GitHub issue number
        adw_id: ADW workflow identifier
        workflow_name: Name of the workflow phase (e.g., "Planning", "Build", "Test")
        status: "success" or "failure" (default: "success")
        artifacts: Dictionary of created artifacts (e.g., {"plan_file": "path", "pr_number": 123})
        next_steps: List of recommended next step descriptions/commands
        worktree_path: Path to the isolated worktree
        additional_info: Workflow-specific additional information (e.g., test results)
    """
    ...
    timestamp = get_local_timestamp()
    message_parts = [
        f"{status_emoji} **{workflow_name} Phase {status_text}**\n",
        f"**Timestamp:** {timestamp}",
        f"**ADW ID:** `{adw_id}`"
    ]

    # Add status with celebration for success
    if status == "success":
        message_parts.append(f"**Status:** {workflow_name} phase completed successfully! 🎉\n")
    else:
        message_parts.append(f"**Status:** {workflow_name} phase encountered errors.\n")

    # Add artifacts section if provided
    if artifacts and len(artifacts) > 0:
        message_parts.append("**Artifacts Created:**")
        for key, value in artifacts.items():
            if value:  # Only show non-empty values
                # Format the key to be more readable
                display_key = key.replace("_", " ").title()
                if isinstance(value, dict):
                    # For nested dicts (like test_results), format nicely
                    message_parts.append(f"- **{display_key}:**")
                    for sub_key, sub_value in value.items():
                        sub_display_key = sub_key.replace("_", " ").title()
                        message_parts.append(f"  - {sub_display_key}: {sub_value}")
                else:
    ...
    # Add worktree location if provided
    if worktree_path:
        message_parts.append(f"**Worktree:** `{worktree_path}`\n")
...
def classify_issue(
    ...
    classification_match = re.search(r"(/chore|/bug|/feature|/refactor|0)", output)
    ...
    if issue_command not in ["/chore", "/bug", "/feature", "/refactor"]:
        return None, f"Invalid command selected: {response.output}"

    # Map /refactor to /chore internally for consistency
    if issue_command == "/refactor":
        logger.info("Mapping /refactor to /chore for workflow execution")
        issue_command = "/chore"
