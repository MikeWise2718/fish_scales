...
def validate_worktree(adw_id: str, state: ADWState) -> Tuple[bool, Optional[str]]:
    ...
    # Note: git worktree list outputs paths with forward slashes on Windows,
    # but state.worktree_path uses backslashes. Normalize for comparison.
    ...
    # Also check if the path exists anywhere in the output (handles relative vs absolute paths)
    worktree_found = (
        normalized_worktree_path in result.stdout or
        worktree_path in result.stdout or
        # Check just the last part (trees/<adw_id>)
        f"trees/{os.path.basename(worktree_path)}" in result.stdout
    )

    if not worktree_found:
        return False, f"Worktree not registered with git (checked: {normalized_worktree_path})"
