...
def main():
    ...
    start_entry = log_execution_start(
        script_name="adw_sdlc_zte_iso.py",
        adw_id=adw_id,
    ...
    try:
        ...
        ship_cmd = ["uv", "run", os.path.join(script_dir, "adw_ship_iso.py"), issue_number, adw_id]
        print(f"\n=== ISOLATED SHIP PHASE (APPROVE & MERGE) ===")
        print(f"Running: {' '.join(ship_cmd)}")

        ship_invocation = track_subprocess_start("adw_ship_iso.py")
        ship = subprocess.run(ship_cmd)
        ship_invocation = track_subprocess_end(ship_invocation, ship.returncode)
        subprocesses.append(ship_invocation)
